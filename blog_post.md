# How I Built an AI System That Predicts India's Electricity Prices
...
1. **Scrapes** daily DAM snapshot data from IEX's public portal (~2,300 days × 96 blocks = 224K+ observations)
...
Three trading strategies were simulated on the same 20-month holdout period. **⚠️ These are backtested results with idealized assumptions — not live trading profits.**
...
- Fixed capacity (100 MW / 50 MW) with no market-price impact
- Perfect execution at actual MCP (no bid-ask spread)
- High Sharpe ratios arise from idealized assumptions
...
**Directional trading** goes long when the model predicts a ≥1% price rise, generating the largest absolute P&L at ₹1,277M — though with more volatility (59.6% daily win rate).
