"""
models/xgboost_model.py — XGBoost Gradient Boosting Model

Primary ML model. Uses early stopping on a time-series validation split.
Saves model + feature importance + full metrics.
...
PREDS_DIR  = os.path.join(os.path.dirname(__file__), "..", "..", "predictions", "xgboost")
...
def run():
    ...
    print_metrics_table("XGBoost (Val)", compute_all_metrics(y_val, y_val_pred))

    y_pred = model.predict(X_test)
    metrics = compute_all_metrics(y_test, y_pred)
    print_metrics_table("XGBoost (Holdout Test)", metrics)
    ...
    # Save
    model.save_model(os.path.join(MODELS_DIR, "xgboost.json"))
    pd.DataFrame([{"model": "xgboost", **metrics}]).to_csv(
