"""
models/random_forest_model.py — Random Forest Regressor

Features: tabular time + lag + rolling features.
Trains one global Random Forest. Competitive baseline vs boosting models.
...
MODELS_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "models", "random_forest"
)
PREDS_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "predictions", "random_forest"
)
...
def run():
    ...
    print_metrics_table("Random Forest (Val)", compute_all_metrics(y_val, y_val_pred))

    y_pred = model.predict(X_test)
    metrics = compute_all_metrics(y_test, y_pred)
    print_metrics_table("Random Forest (Holdout Test)", metrics)
    ...
    pd.DataFrame([{"model": "random_forest", **metrics}]).to_csv(
