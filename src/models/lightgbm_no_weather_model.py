"""
models/lightgbm_no_weather_model.py — LightGBM Model WITHOUT Weather Features

Same as lightgbm_model.py but trained WITHOUT weather data.
...
from config import FEATURE_COLS_NO_WEATHER as FEATURE_COLS
...
MODELS_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "models_no_weather", "lightgbm"
)
PREDS_DIR = os.path.join(
    os.path.dirname(__file__), "..", "..", "predictions", "lightgbm_no_weather"
)


def load_data():
    train = pd.read_parquet(
        os.path.join(DATA_DIR, "training_features_no_weather.parquet")
    )
    test = pd.read_parquet(
        os.path.join(DATA_DIR, "holdout_features_no_weather.parquet")
    )
    return train, test
...
def run():
    ...
    print("=== LightGBM Model (No Weather) ===\n")
    ...
    print_metrics_table(
        "LightGBM No Weather (Val)", compute_all_metrics(y_val, y_val_pred)
    )

    y_pred = model.predict(X_test)
    metrics = compute_all_metrics(y_test, y_pred)
    print_metrics_table("LightGBM No Weather (Holdout Test)", metrics)
    ...
    # Save
    model.booster_.save_model(os.path.join(MODELS_DIR, "lightgbm_no_weather.txt"))
    pd.DataFrame([{"model": "lightgbm_no_weather", **metrics}]).to_csv(
