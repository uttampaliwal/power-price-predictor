...
def check_and_update(source_name, directory, pattern, force=False):
    ...
    if current_time > last_sync or force:
        ...
        return True, "Updated (files changed)"
...
def quick_export():
    ...
    changed2, _ = check_and_update(
        "processed", DATA_PROCESSED_DIR, "*.parquet", force=True
    )

    # Check predictions
    changed3, _ = check_and_update(
        "predictions", PREDS_DIR, "test_predictions.csv", force=True
    )
    ...
    if changed1 or changed2 or changed3:
        ...
        os.system(
            f'"{sys.executable}" "{os.path.join(os.path.dirname(__file__), "powerbi_exporter.py")}"'
        )
    else:
...
if __name__ == "__main__":
    ...
    if args.force:
        ...
        subprocess.run(
            [
                sys.executable,
                os.path.join(os.path.dirname(__file__), "powerbi_exporter.py"),
            ]
        )
    else:
        quick_export()
