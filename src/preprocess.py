...
from config import DATA_PROCESSED_DIR, DATA_RAW_DIR, WEATHER_FEATURE_COLS
...
def add_time_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def hour_bucket(h):
        if h < 6:
            return 0  # off-peak night
        if h < 10:
            return 1  # morning peak
        if h < 14:
            return 2  # afternoon
        if h < 18:
            return 3  # afternoon-pre-evening
        if h < 22:
...
def add_lag_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for window in [7, 30]:
        ...
        df[col] = df.groupby("block")["mcp_rs_per_mwh"].transform(
            lambda x: x.shift(1).rolling(window, min_periods=3).mean()
        )
        col_std = f"mcp_rolling_{window}d_std"
        df[col_std] = df.groupby("block")["mcp_rs_per_mwh"].transform(
...
def merge_weather_data(df: pd.DataFrame, split: str) -> pd.DataFrame:
    ...
    if not os.path.exists(weather_path):
        print(
            f"  [WARN] No weather data found at {weather_path}, proceeding without weather features."
        )
        return df
    ...
    merged = pd.merge(
        df, df_weather, left_on="merge_key", right_on="datetime", how="left"
    )
...
def run(split: str, include_weather: bool = True):
    ...
    feature_cols = [
        c for c in df.columns if c.startswith("mcp_lag") or c.startswith("mcp_rolling")
    ]
    before = len(df)
    # df = df.dropna(subset=feature_cols)
    print(
        f"  (Kept {before} rows. Models will drop the first 7 days internally for training.)"
    )
    ...
    print(f"\n[OK] Saved → {out_path}")
    print(
        f"     Shape: {df.shape}  |  Date range: {df['date'].min().date()} to {df['date'].max().date()}"
    )
